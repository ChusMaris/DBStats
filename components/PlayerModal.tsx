...
const PlayerModal: React.FC<PlayerModalProps> = ({ player, matchStats, matches, movements, esMini, onClose }) => {
  ...
  const getMinutes = (matchId: string | number): number => {
    try {
      if (!movements || !Array.isArray(movements)) return 0;
      const playerMoves = movements.filter(m => String(m.partido_id) === String(matchId) && String(m.jugador_id) === String(player.jugadorId));
      
      const subMoves = playerMoves.filter(m => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start md:items-center justify-center p-4 pt-24 md:p-4 bg-black/60 backdrop-blur-sm transition-all">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[80vh] md:max-h-[90vh] overflow-y-auto flex flex-col animate-fade-in relative">
        <div className="bg-fcbq-blue text-white p-6 flex justify-between items-start sticky top-0 z-10 shadow-md">
          <div className="flex items-center gap-4">
             <div className="w-16 h-16 md:w-20 md:h-20 rounded-full bg-white/20 flex items-center justify-center overflow-hidden border-2 border-white shrink-0">
                <img 
             ...
             <div>
               <h2 className="text-xl md:text-2xl font-bold leading-tight">{player.nombre}</h2>
               <div className="flex flex-wrap gap-2 md:gap-3 text-blue-100 text-xs md:text-sm mt-1">
                 <span className="bg-white/10 px-2 py-0.5 rounded">#{player.dorsal}</span>
                 <span className="bg-white/10 px-2 py-0.5 rounded">{player.partidosJugados} PJ</span>
                 <span className="bg-white/10 px-2 py-0.5 rounded">{player.ppg.toFixed(1)} PPG</span>
               </div>
             </div>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full transition -mr-2 -mt-2"><X size={24} /></button>
        </div>
        <div className="p-4 md:p-6 space-y-6 md:space-y-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                <div className="bg-blue-50 p-3 md:p-4 rounded-lg text-center flex flex-col justify-center"><p className="text-[10px] md:text-xs text-gray-500 uppercase font-bold">Puntos Totales</p><p className="text-xl md:text-2xl font-bold text-fcbq-blue">{player.totalPuntos}</p></div>
                <div className="bg-blue-50 p-3 md:p-4 rounded-lg text-center flex flex-col justify-center"><p className="text-[10px] md:text-xs text-gray-500 uppercase font-bold">Minutos / P</p><p className="text-xl md:text-2xl font-bold text-fcbq-blue">{player.mpg.toFixed(1)}</p></div>
                <div className="bg-blue-50 p-2 rounded-lg flex flex-col items-center justify-center">
                    <p className="text-[10px] md:text-xs text-gray-500 uppercase font-bold mb-1">Tiros Libres %</p>
                    ...
                    <span className="text-[10px] md:text-xs text-gray-500 font-medium mt-1">{player.totalTirosLibresAnotados}/{player.totalTirosLibresIntentados}</span>
                </div>
                <div className="bg-blue-50 p-3 md:p-4 rounded-lg text-center flex flex-col justify-center"><p className="text-[10px] md:text-xs text-gray-500 uppercase font-bold">Puntos / Min</p><p className="text-xl md:text-2xl font-bold text-fcbq-blue">{player.ppm.toFixed(2)}</p></div>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                <div className="bg-white p-3 md:p-4 border rounded-xl shadow-sm">
                    <h3 className="text-base md:text-lg font-bold text-gray-800 mb-4 flex items-center gap-2"><Activity className="w-4 h-4 md:w-5 md:h-5 text-fcbq-accent" /> Evolución Puntos</h3>
                    <div className="h-48 md:h-64">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f3f4f6" />
                          <XAxis dataKey="name" stroke="#9ca3af" tick={{fontSize: 10}} axisLine={false} tickLine={false} dy={10} />
                          <YAxis stroke="#9ca3af" tick={{fontSize: 10}} axisLine={false} tickLine={false} width={25} />
                          <Tooltip 
                ...
                <div className="bg-white p-3 md:p-4 border rounded-xl shadow-sm flex flex-col">
                    <h3 className="text-base md:text-lg font-bold text-gray-800 mb-4 flex items-center gap-2"><TrendingUp className="w-4 h-4 md:w-5 md:h-5 text-fcbq-accent" /> Tiros Libres</h3>
            ...
            <div className="pt-2 pb-4">
              <h3 className="text-lg font-bold text-gray-800 mb-4">Detalle por Partido</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {chartData.map((data, index) => {
                  const stat = data.originalStat;
                  const mins = getMinutes(stat.partido_id);
                  return (
                    <div key={index} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex justify-between items-end mb-3">
                        <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">JORNADA {data.jornadaNum}</span>
                        <span className="text-xl font-bold text-fcbq-blue leading-none">{stat.puntos} <span className="text-xs text-gray-400 font-medium ml-0.5">pts</span></span>
                      </div>
                      ...
                      <div className="flex justify-between items-center text-xs mb-2.5">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="font-bold text-gray-800">{stat.faltas_personales || 0}</span>
                        </div>
                      </div>
